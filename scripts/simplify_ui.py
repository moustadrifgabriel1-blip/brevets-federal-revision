#!/usr/bin/env python3
"""Simplifie l'interface pour n'utiliser que l'analyse IA avec Gemini 2.5 Pro"""

with open('app.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Le bloc à remplacer (avec le choix radio)
old_block = '''    if len(cours_files) == 0:
...
# Nouveau bloc simplifié avec seulement l'IA
new_block = '''    if len(cours_files) == 0:
        st.warning("⚠️ Veuillez d'abord importer vos documents dans l'onglet 'Mes Documents'")
    else:
...
if old_block in content:
    ...
    with open('app.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("✅ Interface simplifiée - Analyse IA uniquement avec Gemini 2.5 Pro")
else:
    print("❌ Bloc non trouvé")
    # Chercher une partie du bloc
    if "Choix du mode d'analyse" in content:
        print("  → 'Choix du mode d'analyse' trouvé")
    if "analysis_mode = st.radio" in content:
        print("  → 'analysis_mode = st.radio' trouvé")
